...
public class HudongBaikeTextExtractor implements TextExtractor{
	...
	public WebNode extractText(WebNode origin) {
		...
		text=PageUtil.deleteSentenceWithKey(text, "参考文献");
		text=PageUtil.deleteSentenceWithKey(text, "相关词条");
		text=PageUtil.deleteSentenceWithKey(text, "相临景点");
		text=PageUtil.deletePiece(text, "分享编辑文章");
		text=PageUtil.replacePiece(text, "纠错编辑摘要", "\n");
		text=PageUtil.replacePiece(text, " 展开收起\n", "\n");
